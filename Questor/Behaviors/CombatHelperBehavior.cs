...
namespace Questor.Behaviors
{
    public class CombatMissionsBehavior
    {
        ...
        private readonly global::DirectEve.DirectEve _directEve;
        ...
        public CombatMissionsBehavior()
        {
            ...
            _directEve = new global::DirectEve.DirectEve();
        ...
        public void ValidateCombatMissionSettings()
        {
            ...
            else
            {
                ...
                _combatMission.AgentId = agent.AgentId;
                _arm.AgentId = agent.AgentId;
                _statistics.AgentID = agent.AgentId;
                AgentID = agent.AgentId;
            }
        }
        public CombatMissionsBehaviorState State { get; set; }       
        ...
        private void TravelToAgentsStation()
        {
           ...
           {
               if (State == CombatMissionsBehaviorState.GotoMission)
               {
                   State = CombatMissionsBehaviorState.ExecuteMission;
               }
               if (State == CombatMissionsBehaviorState.GotoBase)
               {
                   State = CombatMissionsBehaviorState.UnloadLoot;
               }
           
               // Seeing as we are at the destination set traveler.destination to null
        ...
        public void ProcessState()
        {
            ...
            {
                if (State != CombatMissionsBehaviorState.CloseQuestor)
                {
                    BeginClosingQuestor();
                }
            }
            ...
            switch (State)
            {
                case CombatMissionsBehaviorState.Idle:
                    ...
                    if (Cache.Instance.InSpace)
                    {
                        ...
                        LastAction = DateTime.Now;
                        if (State == CombatMissionsBehaviorState.Idle)
                        {
                            State = CombatMissionsBehaviorState.DelayedGotoBase;
                        }
                        break;
                    }
                    ...
                    if (Settings.Instance.AutoStart)
                    {
                        ...
                        if (Settings.Instance.RandomDelay > 0 || Settings.Instance.MinimumDelay > 0)
                        {
                            ...
                            if (State == CombatMissionsBehaviorState.Idle)
                            {
                                State = CombatMissionsBehaviorState.DelayedStart;
                            }
                        ...
                        else
                        {
                            if (State == CombatMissionsBehaviorState.Idle)
                            {
                                State = CombatMissionsBehaviorState.Cleanup;
                            }
                            return;
                        }
                    }
                ...
                case CombatMissionsBehaviorState.DelayedStart:
                    ...
                    //_storyline.Reset();
                    if (State == CombatMissionsBehaviorState.DelayedStart)
                    {
                        State = CombatMissionsBehaviorState.Cleanup;
                    }
                    break;
                ...
                case CombatMissionsBehaviorState.DelayedGotoBase:
                    ...
                    if (State == CombatMissionsBehaviorState.DelayedGotoBase)
                    {
                        State = CombatMissionsBehaviorState.GotoBase;
                    }
                    break;
                ...
                case CombatMissionsBehaviorState.Cleanup:
                    ...
                    {
                        if (State == CombatMissionsBehaviorState.Cleanup)
                        {
                            State = CombatMissionsBehaviorState.GotoBase;
                        }
                        break;
                    ...
                    else
                    {
                        if (State == CombatMissionsBehaviorState.Cleanup)
                        {
                            State = CombatMissionsBehaviorState.CheckEVEStatus;
                        }
                        break;
                    }
                ...
                case CombatMissionsBehaviorState.Start:
                    if (_firstStart && Settings.Instance.MultiAgentSupport)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.Start)
                        {
                            State = CombatMissionsBehaviorState.Switch;
                        }
                        _firstStart = false;
                        break;
                    }
                    ...
                    if (_agentInteraction.State == AgentInteractionState.Idle)
                    {
                        ...
                        //    _storyline.Reset();
                        //    if (State == CombatMissionsModeState.Start)
                        //    {
                        //        State = CombatMissionsModeState.Storyline;
                        //    }
                        //    break;
                        //}
                    ...
                    if (_agentInteraction.State == AgentInteractionState.Done)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.Start)
                        {
                            State = CombatMissionsBehaviorState.Arm;
                        }
                        return;
                    }
                ...
                case CombatMissionsBehaviorState.Switch:
                    ...
                    if (_switch.State == SwitchShipState.Done)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.Switch)
                        {
                            State = CombatMissionsBehaviorState.GotoBase;
                        }
                    }
                ...
                case CombatMissionsBehaviorState.Arm:
                    ...
                    if (_arm.State == ArmState.NotEnoughAmmo)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.Arm)
                        {
                    ...
                    if (_arm.State == ArmState.Done)
                    {
                        ...
                        if (Cache.Instance.CourierMission)
                        {
                            if (State == CombatMissionsBehaviorState.Arm)
                            {
                                State = CombatMissionsBehaviorState.CourierMission;
                            }
                        }
                        else
                        {
                            if (State == CombatMissionsBehaviorState.Arm)
                            {
                                State = CombatMissionsBehaviorState.LocalWatch;
                            }
                        }
                    }
                ...
                case CombatMissionsBehaviorState.LocalWatch:
                    if (Settings.Instance.UseLocalWatch)
                    {
                        ...
                        if (Cache.Instance.LocalSafe(Settings.Instance.LocalBadStandingPilotsToTolerate, Settings.Instance.LocalBadStandingLevelToConsiderBad))
                        {
                            ...
                            {
                                State = CombatMissionsBehaviorState.WarpOutStation;
                            }
                        }
                        else
                        {
                ...
                case CombatMissionsBehaviorState.WaitingforBadGuytoGoAway:
                    ...
                    if (DateTime.Now.Subtract(Cache.Instance.LastLocalWatchAction).TotalMinutes < (int)Time.WaitforBadGuytoGoAway_minutes)
                        break;
                    if (State == CombatMissionsBehaviorState.WaitingforBadGuytoGoAway)
                    {
                        State = CombatMissionsBehaviorState.LocalWatch;
                    }
                    break;
                ...
                case CombatMissionsBehaviorState.WarpOutStation:
                    ...
                    if (warpOutBookmark == null)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.WarpOutStation)
                        {
                            State = CombatMissionsBehaviorState.GotoMission;
                        }
                    }
                ...
                case CombatMissionsBehaviorState.GotoMission:
                  ...
                  //}
                    ...
                    if (_traveler.State == TravelerState.AtDestination)
                    {
                        ...
                        _combatMission.State = MissionControllerState.Start;
                        _combat.State = CombatState.CheckTargets;
                        _traveler.Destination = null;
                    }
                    break;

                case CombatMissionsBehaviorState.CombatHelper:
                    Cache.Instance.LastKnownGoodConnectedTime = DateTime.Now;
                    Cache.Instance.MyWalletBalance = Cache.Instance.DirectEve.Me.Wealth;
                    _combat.ProcessState();
                    _drones.ProcessState();
                ...
                case CombatMissionsBehaviorState.ExecuteMission:
                    ...
                    if (Settings.Instance.DebugStates)
                        Logging.Log("MissionController.State = " + _combatMission.State);
                    ...
                    if (_combat.State == CombatState.OutOfAmmo)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.ExecuteMission)
                        {
                            State = CombatMissionsBehaviorState.GotoBase;
                        }
                    ...
                    if (_combatMission.State == MissionControllerState.Done)
                    {
                        if (State == CombatMissionsBehaviorState.ExecuteMission)
                        {
                            State = CombatMissionsBehaviorState.GotoBase;
                    ...
                    if (_combatMission.State == MissionControllerState.Error)
                    {
                ...
                case CombatMissionsBehaviorState.GotoBase:
                    ...
                    else if (_combat.State != CombatState.OutOfAmmo && Cache.Instance.Mission != null && Cache.Instance.Mission.State == (int)MissionState.Accepted)
                    {
                        if (State == CombatMissionsBehaviorState.GotoBase)
                        {
                            State = CombatMissionsBehaviorState.CompleteMission;
                        }
                    }
                    else
                    {
                ...
                case CombatMissionsBehaviorState.CheckEVEStatus:
                    ...
                    //    Cache.Instance.CloseQuestorCMDExitGame = true;
                    //    Cache.Instance.SessionState = "Exiting";
                    //    if (State == CombatMissionsModeState.CheckEVEStatus)
                    //    {
                    //        BeginClosingQuestor();
                    //    }
                    //    return;
                    ...
                    if (Cache.Instance.TotalMegaBytesOfMemoryUsed > (Settings.Instance.EVEProcessMemoryCeiling - 50) && Settings.Instance.EVEProcessMemoryCeilingLogofforExit != "")
                    {
                        ...
                        if (Settings.Instance.EVEProcessMemoryCeilingLogofforExit == "logoff")
                        {
                            ...
                            if (State == CombatMissionsBehaviorState.CheckEVEStatus)
                            {
                                BeginClosingQuestor();
                            }
                            return;
                    ...
                    if (State == CombatMissionsBehaviorState.CheckEVEStatus)
                    {
                        State = CombatMissionsBehaviorState.Start;
                    }
                    break;

                case CombatMissionsBehaviorState.CloseQuestor:
                    ...
                    if (_traveler.State == TravelerState.AtDestination || DateTime.Now.Subtract(Cache.Instance.EnteredCloseQuestor_DateTime).TotalSeconds > Settings.Instance.SecondstoWaitAfterExteringCloseQuestorBeforeExitingEVE)
                    {
                ...
                case CombatMissionsBehaviorState.CompleteMission:
                    ...
                    if (_agentInteraction.State == AgentInteractionState.Done)
                    {
                        ...
                        if (Cache.Instance.CourierMission)
                        {
                            ...
                            if (State == CombatMissionsBehaviorState.CompleteMission)
                            {
                                State = CombatMissionsBehaviorState.Idle;
                            }
                        }
                    ...
                    if (_unloadLoot.State == UnloadLootState.Done)
                    {
                        ...
                        if (Settings.Instance.AfterMissionSalvaging)
                        {
                            if (Cache.Instance.BookmarksByLabel(Settings.Instance.BookmarkPrefix + " ").Count == 0)
                            {
                                ...
                                //    State = CombatMissionsModeState.Error; //or salvageonly. need to check difference
                                //}
                ...
                case CombatMissionsBehaviorState.GotoSalvageBookmark:
                    ...
                    if (_traveler.State == TravelerState.AtDestination || GateInSalvage())
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.GotoSalvageBookmark)
                        {
                            State = CombatMissionsBehaviorState.Salvage;
                        }
                        _traveler.Destination = null;
                        return;
                    }
                ...
                case CombatMissionsBehaviorState.SalvageUseGate:
                    ...
                    if (closest.Distance < (int)Distance.DecloakRange)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.SalvageUseGate)
                        {
                            State = CombatMissionsBehaviorState.SalvageNextPocket;
                        }
                ...
                case CombatMissionsBehaviorState.SalvageNextPocket:
                    ...
                    else //we have not moved to the next pocket quite yet
                    {
                        if (DateTime.Now.Subtract(_lastPulse).TotalMinutes > 2)
                        {
                            ...
                            if (State == CombatMissionsBehaviorState.SalvageNextPocket)
                            {
                                State = CombatMissionsBehaviorState.SalvageUseGate;
                            }
                        }
                ...
                case CombatMissionsBehaviorState.Storyline:
                    ...
                    if (_storyline.State == StorylineState.Done)
                    {
                        ...
                        if (State == CombatMissionsBehaviorState.Storyline)
                        {
                            State = CombatMissionsBehaviorState.GotoBase;
                        }
                        break;
                ...
                case CombatMissionsBehaviorState.CourierMission:
                    ...
                    if (_courier.State == CourierMissionState.Done)
                    {
                        ...
                        Cache.Instance.CourierMission = false;
                        if (State == CombatMissionsBehaviorState.CourierMission)
                        {
                            State = CombatMissionsBehaviorState.GotoBase;
                        }
                    }
                ...
                case CombatMissionsBehaviorState.GotoNearestStation:
                    ...
                    else
                    {
                        if (State == CombatMissionsBehaviorState.GotoNearestStation)
                        {
                            State = CombatMissionsBehaviorState.Error; //should we goto idle here?
                        }
                    }
                    break;
            }
